...
- **CI cannot catch a Railway Node floor.** `.nvmrc` is the bare string `22` and CI uses `node-version-file: .nvmrc`, so `setup-node` resolves the latest 22.x and passes. A package whose `engines.node` excludes Railway's actual version goes green in CI and fails at deploy — Prisma 7 would fail during `npm ci`, before the build phase, because its `preinstall` gate calls `process.exit(1)`.
