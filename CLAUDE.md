...
- **Prisma 7 does NOT clear the `deepmerge-ts` advisory** that `npm audit` reports against `prisma`/`@prisma/config`. npm's advisory range includes 7.9.1 and its `fixAvailable` is a downgrade to 6.12.0. The impact is nil here — it is `deepmerge` used as the merger for CLI config loading with every remote input disabled — but `npm audit` stays at 3 high until Prisma drops the dependency, so do not read that as a regression.
